...
// Enhanced AI Orb with click reactions, white eyes (no pupils), speech bubbles, original animations
export const AIOrb = memo(function AIOrb({ 
  ...
  return (
    <div className="relative">
      {/* Speech Bubble - Blended, no stroke */}
      <AnimatePresence>
        {showBubble && (
          <motion.div
          ...
          >
            <div className="bg-[var(--card)]/80 backdrop-blur-sm px-4 py-2 rounded-xl shadow-xl">
              <p className="text-sm font-medium text-[var(--foreground)]">{bubbleText}</p>
            </div>
            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-0 border-l-8 border-r-8 border-t-8 border-transparent border-t-[var(--card)]/80" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        ...
        {/* Energy particles - chaotic dots (not solid lines) */}
        {[...Array(8)].map((_, i) => {
          const angle = (i / 8) * 360;
