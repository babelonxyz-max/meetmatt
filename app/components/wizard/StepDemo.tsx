...
export function StepDemo({ agentName, personality, onContinue }: StepDemoProps) {
  ...
  return (
    ...
    >
      <div className="mb-1.5 flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[0.14em]">
        <span className="rounded-full border border-white/12 bg-white/[0.05] px-2.5 py-1 text-white/70">
          Tone: {toneLabel}
        </span>
        <span className="rounded-full border border-[#ffaa44]/20 bg-[#ffaa44]/10 px-2.5 py-1 text-[#ffe3c5]">
        ...
        <div className="mt-2 rounded-[1rem] border border-white/10 bg-black/16 p-2">
          <div className="flex gap-2">
            <input
              ...
              disabled={remaining <= 0 || isBotTyping}
              className="brand-button-secondary flex-1 rounded-xl px-3 py-2 text-sm text-white placeholder:text-white/40 focus:border-[#ffaa44] focus:outline-none disabled:opacity-50"
          ...
          <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-[11px] leading-relaxed text-white/55">{helperText}</p>
            <button
              type="button"
              onClick={onContinue}
              disabled={!canContinue}
              className="brand-button px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] disabled:cursor-not-allowed disabled:opacity-40"
            >
